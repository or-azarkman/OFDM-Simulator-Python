...
OFDM baseband transceiver: theoretical BER (AWGN), Monte Carlo BER vs SNR, QPSK and 16-QAM, AWGN and multipath channels, ZF/MMSE equalization, EVM (Error Vector Magnitude), unit tests.
...
Results: `results/5000_symbols/` (AWGN), `results/5000_symbols_multipath_zf/` or `_mmse/` (multipath); each has `images/` and CSV (BER + EVM). Comparison: `py simulations/plot_ber_comparison.py` (BER), `py simulations/plot_evm_comparison.py` (EVM), `py simulations/plot_constellation_comparison.py` (4×3 constellation). Commands: `docs/RUN_AND_TEST.md`.
...
- **Comparison table:** `results/summary/comparison_table.csv` (BER), `evm_comparison_table.csv` (EVM); plus `.md` variants. BER/EVM per SNR for AWGN, Multipath (no eq), ZF, MMSE. Generated by `plot_ber_comparison.py` and `plot_evm_comparison.py`.
...
Baseband chain: bits → modulation → IFFT → CP; receiver: CP removal → FFT → ZF/MMSE equalization (multipath) → demod → BER. EVM measures received vs transmitted symbol deviation (%). CP enables circular convolution and one-tap equalization. Gray coding; theoretical BER (AWGN). Tests: transmitter, receiver, channel, theory, equalizers, evm. For lessons learned and next-phase roadmap, see `docs/LESSONS_LEARNED.md` and `docs/NEXT_PHASE_PLAN.md`.
...
Tests cover: transmitter, receiver, channel (AWGN + multipath), theory, equalizers (ZF, MMSE), EVM.
...
OFDM PHY-layer simulation. Mathematical details: `docs/ofdm_overview.md`. Lessons learned, limitations, and future work: `docs/LESSONS_LEARNED.md`. Next phase (pilots, CFO, STO): `docs/NEXT_PHASE_PLAN.md`.
